...
- [x] Tạo `internal/platform/cache.Cache` interface.
- [x] Thêm method `Get(ctx, key, dest) error`.
- [x] Thêm method `Set(ctx, key, value, ttl) error`.
- [x] Thêm method `Delete(ctx, keys ...string) error`.
- [x] Thêm method `Exists(ctx, key) (bool, error)`.
- [x] Thêm method `WithLock(ctx, key, ttl, fn) error`.
- [x] Thêm method `Close() error`.
- [x] Chuẩn hóa lỗi `ErrCacheMiss`.
- [x] Implement Redis adapter.
- [x] Redis adapter encode/decode JSON cho object cache.
- [x] Redis lock dùng `SET NX PX` và release an toàn bằng value owner token.
- [x] Unit test cache mock contract.

### 3.2. Database abstraction

Checklist:

- [x] Tạo `internal/platform/database.Database` interface.
- [x] Thêm `FindOne(ctx, collection string, filter any, dest any, opts ReadOptions) error`.
- [x] Thêm `FindMany(ctx, collection string, filter any, dest any, opts ReadOptions) error`.
- [x] Thêm `InsertOne(ctx, collection string, document any, opts WriteOptions) error`.
- [x] Thêm `UpdateOne(ctx, collection string, filter any, update any, opts WriteOptions) error`.
- [x] Thêm `DeleteOne(ctx, collection string, filter any, opts WriteOptions) error`.
- [x] Không dùng variadic option mơ hồ cho behavior quan trọng; dùng typed options `ReadOptions` và `WriteOptions` có zero-value an toàn.
- [x] `ReadOptions` gồm `CacheKey`, `CacheTTL`, `LockOnMiss`.
- [x] `WriteOptions` gồm `LockKey`, `InvalidateKeys`, `StrictLock`.
- [x] Validate options trước khi chạy query; option invalid phải log warning hoặc trả lỗi theo environment policy.
- [x] Implement `MongoDatabase` chỉ xử lý MongoDB.
- [x] Implement `CachedDatabase` wrap `MongoDatabase` và `Cache`.
- [x] Repository luôn nhận `database.Database`, không nhận Mongo client trực tiếp.
- [x] Database abstraction chịu trách nhiệm read-through cache.
- [x] Database abstraction chịu trách nhiệm invalidation sau write.
- [x] Database abstraction chỉ dùng Redis lock khi `ReadOptions.LockOnMiss=true`.
- [x] Nếu read lock fail và `LockOnMiss=true`, fallback mặc định là đọc thẳng MongoDB, log warning, không fail API.
- [x] Database abstraction dùng write lock/invalidation để tránh stale read sau write.
- [x] Nếu write lock fail và `WriteOptions.StrictLock=true`, write fail với `DEPENDENCY_ERROR`; nếu `StrictLock=false`, ghi MongoDB rồi invalidate best-effort.
...
- [x] Tạo `UserRepository` trong domain.
- [x] `UserRepository.Create(ctx, user) error`.
- [x] `UserRepository.FindByID(ctx, id) (*User, error)`.
- [x] `UserRepository.FindByEmail(ctx, email) (*User, error)`.
- [x] `UserRepository.UpdateLastLogin(ctx, userID, at) error`.
- [x] Tạo `SessionRepository` trong domain auth.
- [x] `SessionRepository.Create(ctx, session) error`.
- [x] `SessionRepository.FindActiveByID(ctx, sessionID) (*Session, error)`.
- [x] `SessionRepository.FindByRefreshTokenHash(ctx, hash) (*Session, error)`.
- [x] `SessionRepository.RotateRefreshToken(ctx, sessionID, oldHash, newHash, expiresAt) error`.
- [x] `SessionRepository.Revoke(ctx, sessionID, reason, revokedAt) error`.
- [x] `SessionRepository.RevokeAllByUserID(ctx, userID, reason, revokedAt) error`.
- [x] `SessionRepository.ListActiveByUserID(ctx, userID) ([]Session, error)`.
- [x] Tạo `LoginHistoryRepository`.
- [x] `LoginHistoryRepository.Append(ctx, event) error`.
- [x] `LoginHistoryRepository.ListByUserID(ctx, userID, limit, offset) ([]LoginHistory, error)`.

### 3.4. Service interfaces

Checklist:

- [x] Tạo `AuthService` interface.
- [x] `Register(ctx, input) (*AuthResult, error)`.
- [x] `Login(ctx, input, meta) (*AuthResult, error)`.
- [x] `Refresh(ctx, refreshToken, meta) (*AuthResult, error)`.
- [x] `Logout(ctx, accessToken, sessionID) error`.
- [x] `LogoutAll(ctx, userID) error`.
- [x] `ListDevices(ctx, userID) ([]DeviceSession, error)`.
- [x] `ListLoginHistory(ctx, userID, pagination) ([]LoginHistory, error)`.
- [x] Tạo `TokenService` interface.
- [x] `GenerateAccessToken(ctx, claims) (string, time.Time, error)`.
- [x] `ValidateAccessToken(ctx, token) (*AccessClaims, error)`.
- [x] `GenerateRefreshToken() (plain string, hash string, error)`.
- [x] `HashRefreshToken(plain string) string`.
- [x] `BlacklistAccessToken(ctx, tokenID, ttl) error`.
- [x] `IsAccessTokenBlacklisted(ctx, tokenID) (bool, error)`.
...
- [x] Tách access log và application/error log nếu config bật.
...
- [x] Tạo `MonitoringService` interface.
- [x] `GetSystemStatus(ctx) (*SystemStatus, error)`.
- [x] `GetRuntimeMetrics(ctx) (*RuntimeMetrics, error)`.
- [x] `GetDependencyStatus(ctx) (*DependencyStatus, error)`.
- [x] `GetAuthStats(ctx, from, to) (*AuthStats, error)`.
- [x] `GetRecentErrors(ctx, pagination) ([]ErrorEvent, error)`.
- [x] `GetRecentAuditLogs(ctx, pagination) ([]AuditLog, error)`.
- [x] Tạo `AuditLogRepository`.
- [x] `AuditLogRepository.Append(ctx, event) error`.
- [x] `AuditLogRepository.List(ctx, filter, pagination) ([]AuditLog, error)`.
- [x] Monitoring service chỉ dùng abstraction/repository, không gọi Mongo/Redis client trực tiếp.
...
- [x] Tạo `Outbox` interface: `Enqueue(ctx, event) error`, `ClaimBatch(ctx, limit) ([]Event, error)`, `MarkDone(ctx, id) error`, `MarkFailed(ctx, id, reason) error`.
- [x] `OutboxEvent` gồm `ID`, `IdempotencyKey`, `Type`, `Payload`, `MaxRetries`, `RetryCount`, `Status`, `ProcessAfter`, `CreatedAt`, `UpdatedAt`.
- [x] Caller sinh `ID`; với event có business dedup phải sinh `IdempotencyKey` deterministic, ví dụ `login:{user_id}:{request_id}` hoặc `audit:{request_id}:{action}`.
- [x] Mongo unique index trên `idempotency_key` để retry ở layer trên không tạo duplicate event.
...
- [x] Tạo `RevokedTokenRepository` lưu `jti`, `user_id`, `session_id`, `expires_at`, `revoked_at`.
- [x] `RevokedTokenRepository` có TTL index theo `expires_at`.
- [x] Token blacklist check Redis trước, nếu miss thì fallback Mongo `revoked_tokens`.
- [x] Khi Redis recover, có thể warm lại blacklist từ Mongo theo token chưa expired.
...
- [x] Tạo `User` entity gồm `ID`, `Email`, `PasswordHash`, `Name`, `Roles`, `Status`, `FailedLoginAttempts`, `LockedUntil`, `CreatedAt`, `UpdatedAt`, `LastLoginAt`.
- [x] Email unique, normalized lowercase.
- [x] Password không bao giờ lưu plain text.
- [x] User roles hỗ trợ tối thiểu `user`, `admin`.
- [x] User role mặc định khi register là `user`; `admin` chỉ được seed/cấp qua admin path riêng, không qua public register.
- [x] User status hỗ trợ tối thiểu `active`, `disabled`.
- [x] Mongo index unique cho `email`.

### 4.2. Session/device

Checklist:

- [x] Tạo `Session` entity gồm `ID`, `UserID`, `RefreshTokenHash`, `RefreshTokenExpiresAt`, `DeviceID`, `DeviceName`, `UserAgent`, `IP`, `TokenFamilyID`, `RevokedAt`, `RevokedReason`, `LastSeenAt`, `CreatedAt`, `UpdatedAt`.
- [x] `DeviceID` lấy từ header/body nếu client gửi, nếu không server sinh UUID.
- [x] `DeviceID` chỉ là hint phục vụ UX/hiển thị thiết bị, không dùng làm lookup key bảo mật.
- [x] Validate `DeviceID` là UUID v4 hoặc server-generated UUID; reject/truncate input quá dài.
- [x] Session lookup bảo mật luôn dựa trên `session_id` + refresh token hash hoặc access token claims, không dựa trên `device_id`.
- [x] Một login tạo một session mới.
- [x] Refresh token rotation cập nhật hash mới và expiry mới trên session hiện tại.
- [x] `TokenFamilyID` sinh một lần khi login tạo session mới và bất biến trong toàn bộ refresh-token chain của session đó.
- [x] `RotateRefreshToken` phải atomic theo điều kiện `session_id + old_refresh_hash + revoked_at nil + expires_at > now`.
- [x] Nếu atomic update không match document nào, đọc lại session để phân biệt invalid/reuse/race/logout.
- [x] Nếu session active nhưng `RefreshTokenHash` đã khác old hash, coi là stale token/reuse sau rotation và revoke toàn bộ session cùng `TokenFamilyID`.
- [x] Nếu session đã revoked do logout, trả token invalid/revoked bình thường, không coi là reuse attack và không revoke thêm cả family.
- [x] Nếu session expired, trả token expired, không coi là reuse attack.
- [x] Refresh bằng token cũ sau rotation phải fail.
- [x] Rotation race hai request dùng cùng refresh token: một request thắng atomic update, request còn lại bị stale old hash; policy mặc định revoke family để bảo thủ, nhưng phải audit `refresh_reuse_suspected`.
- [x] Logout set `RevokedAt` và `RevokedReason`.
- [x] Logout all revoke mọi session active theo user.
- [x] Mongo index cho `user_id`, `refresh_token_hash`, `device_id`, `revoked_at`.

### 4.3. Login history

Checklist:

- [x] Tạo `LoginHistory` entity gồm `ID`, `UserID`, `Email`, `Success`, `FailureReason`, `IP`, `UserAgent`, `DeviceID`, `CreatedAt`.
- [x] Ghi history cho login thành công.
- [x] Ghi history cho login thất bại khi xác định được email.
- [x] Không ghi password hoặc token vào history.
- [x] Mongo index cho `user_id`, `email`, `created_at`.

### 4.4. Audit log và error event

Checklist:

- [x] Tạo `AuditLog` entity gồm `ID`, `ActorUserID`, `Action`, `ResourceType`, `ResourceID`, `IP`, `UserAgent`, `RequestID`, `Metadata`, `CreatedAt`.
- [x] `AuditLog.Metadata` mặc định dùng `map[string]string` cho phase đầu để dễ redact/index; nếu cần payload phức tạp thì tạo typed metadata riêng theo action.
- [x] Ghi audit log cho login, logout, logout all, refresh token reuse failure, user disabled/forbidden access.
- [x] Không lưu token/password/secret trong metadata.
- [x] Tạo `ErrorEvent` view/model cho monitoring gồm `RequestID`, `ErrorCode`, `Message`, `Cause`, `Stack`, `Path`, `Method`, `Status`, `UserID`, `CreatedAt`.
- [x] Error event có thể lấy từ log file parser hoặc repository tùy implementation phase đầu tiên; mặc định phase đầu lưu event quan trọng vào Mongo qua audit/error repository.
- [x] Nếu audit/error/history write trực tiếp fail, enqueue vào outbox để retry thay vì mất event vĩnh viễn.
- [x] Mongo index cho `request_id`, `action`, `actor_user_id`, `created_at`, `error_code`.
...
- [x] Trả lỗi JSON thống nhất cho unauthenticated/forbidden.
...
- [x] `GET /v1/admin/monitoring/status`.
- [x] Trả trạng thái app, uptime, version, environment.
- [x] `GET /v1/admin/monitoring/dependencies`.
- [x] Trả trạng thái MongoDB, Redis, cache wrapper.
- [x] `GET /v1/admin/monitoring/runtime`.
- [x] Trả goroutines, memory, GC, CPU nếu có collector hỗ trợ.
- [x] `GET /v1/admin/monitoring/auth-stats`.
- [x] Trả số login success/failure, active sessions, revoked sessions theo khoảng thời gian.
- [x] `GET /v1/admin/monitoring/errors`.
- [x] Trả lỗi gần đây, filter theo `error_code`, `status`, `request_id`.
- [x] `GET /v1/admin/monitoring/audit-logs`.
- [x] Trả audit log gần đây, filter theo actor/action/resource/time.
- [x] Toàn bộ `/v1/admin/*` yêu cầu auth middleware.
- [x] Toàn bộ `/v1/admin/*` yêu cầu admin guard dựa trên `roles` của user.
- [x] `GET /metrics` expose Prometheus metrics, có thể public nội bộ hoặc bảo vệ bằng config.

## 6. Database/cache consistency plan

Checklist:

- [x] Quy ước cache key tập trung trong database/repository option helper.
- [x] User by id key: `user:id:{id}`.
- [x] User by email key: `user:email:{email}`.
- [x] Session by id key: `session:id:{id}`.
- [x] Session by refresh hash key: `session:refresh:{hash}`.
- [x] Active devices key: `session:user:{user_id}:active`.
- [x] Read path:
  - [x] Nếu có `CacheKey`, đọc cache trước.
  - [x] Cache miss chỉ acquire lock `lock:{CacheKey}` nếu `ReadOptions.LockOnMiss=true`.
  - [x] Sau khi có lock, kiểm tra cache lại.
  - [x] Nếu lock fail theo fallback policy hoặc vẫn miss sau lock, đọc MongoDB.
  - [x] Set cache với TTL.
  - [x] Release lock.
...
- [x] Không tự sinh cache key từ raw filter phức tạp.
- [x] Write path:
  - [x] Nếu có `LockKey`, acquire write lock.
  - [x] Ghi MongoDB trước.
  - [x] Invalidate `InvalidateKeys`.
  - [x] Release lock.
- [x] Khi cache lỗi, không làm API fail nếu MongoDB vẫn đọc/ghi được, trừ lock critical path được cấu hình strict.
- [x] Với write nhạy cảm như refresh token rotation, dùng conditional atomic update thay vì read-then-write.
- [x] Repository chịu trách nhiệm truyền `InvalidateKeys` cụ thể cho từng write operation.
- [x] Khi refresh token rotation, repository truyền invalidate keys tối thiểu: `session:id:{id}`, `session:refresh:{old_hash}`, `session:user:{user_id}:active`.
- [x] Sau rotation thành công, cache key `session:refresh:{new_hash}` chỉ được populate từ lần đọc tiếp theo hoặc set có kiểm soát.
- [x] Register/login có nhiều write thì dùng Mongo transaction nếu deployment hỗ trợ replica set.
- [x] Nếu transaction không available, audit/history write trực tiếp là best-effort và không làm fail auth chính.
- [x] Nếu audit/history/error write trực tiếp fail, enqueue outbox event để retry at-least-once.
- [x] Outbox worker idempotent theo `IdempotencyKey`/business target để tránh duplicate side effects.
- [x] Database abstraction không expose Mongo client/collection ra repository.
- [x] Nếu repository dùng `bson.M`, giới hạn trong package `internal/repository/mongo`, không leak vào domain/service/handler.
...
- [x] Thêm Mongo config: `MONGO_MAX_POOL_SIZE`, `MONGO_MIN_POOL_SIZE`, `MONGO_CONNECT_TIMEOUT`, `MONGO_READ_PREFERENCE`.
...
- [x] `cmd/api/main.go` load config.
- [x] Init logger.
- [x] Logger ghi ra terminal theo config.
- [x] Logger ghi ra file theo config.
- [x] Tạo request id generator.
- [x] Init Prometheus registry/collectors.
- [x] Init outbox worker nếu enabled.
- [x] Connect MongoDB.
- [x] Connect Redis.
- [x] Tạo cache adapter.
- [x] Tạo mongo database adapter.
- [x] Wrap bằng cached database.
- [x] Chạy migration/bootstrap schema/index trước khi serve traffic theo policy.
- [x] Ensure Mongo indexes.
- [x] Log index created/exists/fail; index bắt buộc fail thì startup fail, index phụ trợ fail thì log error theo policy.
- [x] Wire repositories.
- [x] Wire services.
- [x] Wire monitoring service.
- [x] Wire admin guard middleware.
- [x] Wire Prometheus middleware và `/metrics`.
- [x] Wire handlers/router.
- [x] Start HTTP server.
...
- [x] Nếu bật debug body logging, phải redact password/token/secret/email nếu config yêu cầu.
...
- [x] Tạo middleware `AdminGuard` kiểm tra role `admin` từ user context/session.
- [x] Tạo shared `Pagination` input gồm `limit`, `offset` hoặc cursor, `sort`, `from`, `to` khi cần filter thời gian.
- [x] Cursor pagination mặc định dùng `_id`/`created_at` based cursor ổn định; không dùng skip/offset cho collection lớn nếu endpoint có thể tăng dữ liệu mạnh.
- [x] Áp max limit mặc định cho list endpoint, ví dụ 100 records/request.
- [x] Chuẩn hóa response list gồm `items`, `limit`, `offset`, `total` nếu query tính được.
- [x] Tạo `internal/platform/ratelimit.Limiter` interface để có thể mock khi test.
- [x] Implement Redis-backed rate limiter adapter.
- [x] Áp rate limit cho `POST /v1/auth/login` theo IP + email.
- [x] Áp rate limit cho `POST /v1/auth/register` theo IP.
- [x] Áp rate limit cho `POST /v1/auth/refresh` theo IP + user/session nếu xác định được.
- [x] Rate limit trả `429 TOO_MANY_REQUESTS` với error code ổn định.
...
- [x] Refresh token sinh bằng crypto random.
- [x] Refresh token chỉ lưu hash.
- [x] Access JWT có `sub`, `sid`, `jti`, `iat`, `exp`.
- [x] JWT secret đọc từ env.
- [x] JWT header có `kid`; token service ký bằng current key và validate bằng current rồi fallback previous nếu configured.
- [x] Refresh token rotation bắt buộc.
- [x] Refresh token reuse sau rotation fail.
- [x] Logout revoke session và blacklist access token.
- [x] Logout ghi revoked access token `jti` vào Redis và Mongo `revoked_tokens` với TTL index.
- [x] Blacklist validation Redis miss thì fallback Mongo để chịu được Redis restart.
- [x] Không log password/token.
- [x] Validate input bằng validator.
...
- [x] Account lockout policy: sau N lần login fail trong T phút thì tạm khóa bằng Redis hoặc `LockedUntil`; nếu chưa implement captcha, ghi rõ trong README.
- [x] Monitoring endpoints nằm sau auth và admin guard.
- [x] Admin guard kiểm tra role `admin`; user thường không truy cập được `/v1/admin/*`.
- [x] Rate limit login/register/refresh bằng Redis theo IP và email/user id.
- [x] Logs và audit logs phải redact secret/token/password.

## 14. Testing checklist

### 14.1. Unit tests

- [x] Cache Redis adapter: get/set/delete/cache miss.
- [x] Cache lock: chỉ một caller giữ lock.
- [x] Cached database read-through: miss -> Mongo -> set cache.
- [x] Cached database cache hit: không gọi Mongo.
- [x] Cached database stampede: concurrent miss chỉ gọi Mongo một lần.
- [x] Cached database write: gọi Mongo rồi invalidate keys.
- [x] User repository: create/find/update dùng database abstraction đúng options.
- [x] Session repository: create/find/rotate/revoke/list active.
- [x] Auth service register: hash password và tạo user.
- [x] Auth service login success: tạo session, token, history.
- [x] Auth service login failure: trả unauthorized và ghi history.
- [x] Auth service refresh: rotate refresh token.
- [x] Auth service refresh token reuse: fail.
- [x] Auth service logout: revoke session và blacklist token.
- [x] Token service: generate/validate/expired/blacklist.
- [x] Logger writes console/file according to config.
- [x] Logger redacts password/token/secret fields.
- [x] Error mapper maps domain/app errors to correct HTTP status and code.
- [x] Recovery middleware converts panic to `INTERNAL_ERROR`.
- [x] Monitoring service returns system/runtime/dependency/auth stats from mocked dependencies.
- [x] Admin guard allows role `admin` and rejects role `user`.
- [x] Rate limiter allows under-limit requests and rejects over-limit requests.
- [x] Refresh token rotation atomic update rejects stale old hash.
- [x] Refresh token reuse revokes token family.
- [x] Prometheus collectors register without duplicate registration panic.
- [x] Context keys không conflict giữa middleware/logger/service.
- [x] Database read/write options validate `ReadOptions`/`WriteOptions` và lock fallback đúng policy.
- [x] `FindMany` không cache nếu không có explicit `CacheKey`.
- [x] `FindMany` chỉ cache với explicit normalized `CacheKey` và `CacheableFilter`.
- [x] Session rotation invalidates `session:id`, old `session:refresh`, and active device list keys.
- [x] Outbox enqueue/drain/retry handles temporary audit write failure.
- [x] Outbox unique `IdempotencyKey` ngăn duplicate khi caller retry sau timeout.
...
- [x] Refresh success.
- [x] Refresh with old token fails.
- [x] Logout success.
- [x] Protected endpoint without token returns 401.
- [x] Protected endpoint with blacklisted token returns 401.
- [x] Devices endpoint returns active devices.
...
- [x] Ghi danh sách endpoint và request/response mẫu.
- [x] Ghi kiến trúc layer và dependency direction.
- [x] Ghi cách mock database/cache/repository/service khi test.
- [x] Ghi policy refresh token rotation và logout invalidation.
- [x] Ghi logging config, log format, log file path, rotation policy.
...
- [x] Phase 1: Project bootstrap, config, logger, Docker Compose, context keys, seed/migrate skeleton.
- [x] Phase 2: Error model, response format, recovery/error middleware, request id, logging middleware, CORS/body-limit/timeout middleware.
- [x] Phase 3: Cache interface + Redis implementation + lock/rate-limit primitives + tests.
- [x] Phase 4: Database interface + Mongo adapter + typed `ReadOptions`/`WriteOptions` + cached database wrapper + degradation tests.
- [x] Phase 5: Domain entities + repository interfaces + cache key helpers + pagination/cursor primitives.
- [x] Phase 6: Mongo repository implementations + index bootstrap + migration/seed commands.
- [x] Phase 7: Token service only: JWT `kid`, current/previous key, `NotAfter`, refresh token crypto/hash, blacklist abstraction.
- [x] Phase 8: Auth service skeleton + Gin router + auth endpoints với thin handlers để có endpoint test thủ công sớm.
- [x] Phase 9: Hoàn thiện auth: rate limit, ETag, refresh rotation, token family reuse handling, outbox, revoked-token Mongo fallback.
- [x] Phase 10: RBAC/admin guard + user/device/login-history endpoints + account lockout/anomaly audit policy.
- [x] Phase 11: Monitoring service + audit/error event repository + Prometheus collectors + admin monitoring endpoints + `/metrics`.
- [x] Phase 12: Integration tests and README.
- [x] Phase 13: Hardening pass: security, errors, logging, graceful shutdown, lint, race/vuln checks, Docker hardening.
...
- [x] Tất cả exported type/function quan trọng có godoc.
- [x] Dependency được pin version trong `go.sum`.
- [x] Docker image build bằng non-root user.
- [x] Secret không được bake vào Docker image hoặc log startup.
...
- [x] `go test ./...` pass.
- [x] API chạy được bằng config local.
- [x] MongoDB và Redis chạy được qua Docker Compose.
- [x] Register/login/refresh/logout hoạt động end-to-end.
- [x] Access token logout xong không dùng lại được.
- [x] Refresh token cũ sau rotation không dùng lại được.
- [x] User xem được danh sách thiết bị đang đăng nhập.
- [x] User xem được lịch sử đăng nhập.
- [x] Repository không phụ thuộc Redis hoặc Mongo client cụ thể.
- [x] Service không phụ thuộc Gin hoặc database driver.
- [x] Handler không chứa business logic.
- [x] Database/cache có thể mock trong unit test.
- [x] Cache coordination nằm dưới database abstraction, không nằm trong repository.
- [x] Log được ghi ra terminal.
- [x] Log được ghi ra file theo config.
- [x] Mọi response lỗi có error code và request id.
- [x] Panic được recovery, log stack/cause, và trả response an toàn.
...
- [x] DeviceID chỉ là UX hint, được validate, và không dùng làm security lookup.
- [x] JWT `kid` rotation hỗ trợ current/previous key và previous key `NotAfter`.
